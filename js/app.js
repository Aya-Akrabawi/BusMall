...
imgSection.addEventListener('click',click)
var totalClicks = 0
function click(){
    if (totalClicks < 25 ){
      ...
      if(clickedElementId === 'leftImg' || clickedElementId === 'midImg' || clickedElementId === 'rightImg' ){
            ...
            if(clickedElementId === 'midtImg'){
                allGoats[midIndex].numberOfClicks +=1;
            }

            if(clickedElementId === 'leftImg'){
                allGoats[rightIndex].numberOfClicks +=1;
            }

            randomImg();
         } else {
       // generateUserMessage();
        imgSection.removeEventListener('click', goatClickHandler);
    }
}}
...
function generateUserMessage(){   
    for (let index = 1; index <= collections.length; index++) {
        var list = document.createElement('li')
    votes.appendChild(list)
    list.textContent ='Image ' + index +': '+ collections[index].numberOfTimesShown  
    } 
